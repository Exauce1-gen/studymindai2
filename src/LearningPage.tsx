...
async function generateQuiz(text: string): Promise<Question[]> {
  const systemPrompt = `Tu es un professeur expert. Génère EXACTEMENT 5 questions QCM DIFFÉRENTES en français.
...
Format JSON strict :
{
  "questions": [
    {
      "question": "Question 1 sur le premier concept ?",
      "options": ["Vraie réponse A", "Fausse réponse B", "Fausse réponse C", "Fausse réponse D"],
      "correct": 0,
      "explanation": "Explication détaillée"
    },
    {
      "question": "Question 2 sur un AUTRE concept ?",
      "options": ["Fausse réponse", "Vraie réponse", "Fausse réponse", "Fausse réponse"],
      "correct": 1,
      "explanation": "Explication détaillée"
    }
  ]
...
RÈGLES ABSOLUES :
1. CHAQUE question doit porter sur un CONCEPT DIFFÉRENT du cours
2. Question 1 : concept A, Question 2 : concept B, Question 3 : concept C, etc.
3. Les options doivent être DIFFÉRENTES pour chaque question
4. "options" = 4 réponses COMPLÈTES (jamais "Option A" ou "Option B")
5. "correct" = index 0-3 de la bonne réponse
6. Génère 5 questions VARIÉES sur 5 concepts DIFFÉRENTS`;

  const userMessage = `Voici le cours. Crée 5 questions QCM sur 5 CONCEPTS DIFFÉRENTS :\n\n${text}`;
  ...
  try {
    ...
    console.log('Quiz JSON:', cleanedResponse);
...
- "options" = 4 VRAIES réponses (PAS "Option A")
...
- Questions de difficulté croissante`;
      ...
      console.log('Examen JSON:', cleanedResponse);
      ...
      alert(`Impossible de générer l'examen. Réessayez avec un cours plus court.`);
    } finally {
      setLoading(false);
    }
  };
